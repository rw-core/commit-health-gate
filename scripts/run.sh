#!/usr/bin/env bash
#
# Fast path for the commit-health-gate composite action.
#
...
fallback() {
  echo "commit-health-gate: $1, falling back to a source build." >&2
...
asset="commit-health-gate-${os}-${arch}${ext}"
repo="${ACTION_REPO:-}"
ref="${ACTION_REF:-}"
# Local (`uses: ./`) or vendored copies have no release to download from.
[ -n "$repo" ] && [ -n "$ref" ] || fallback "no release ref (local/vendored action)"
# CHG_BASE_URL lets a GHES mirror (or tests) override the release host.
base_url="${CHG_BASE_URL:-https://github.com/${repo}/releases/download/${ref}}"
